...
class Custom_Uploader_Uploader {
	...
	function create_short_url($attach_id) {
        $cup_options = get_option( 'cup_options' );
        $google_url_shortener_api_key = $cup_options['cup_google_url_shortener_api_key'];
		$google_url_shortener_is_enabled = $cup_options['cup_google_url_shortener_is_enabled'];

		if($google_url_shortener_is_enabled) {
	        // Create short URL
	        $url_shortner = new GoogleUrlShortener($google_url_shortener_api_key);
	        $url_shortner->generate_short_url(get_attachment_link($attach_id));
			$short_url = $url_shortner->get_short_url();
    
	        if(strlen($short_url) > 0) {
	            update_post_meta($attach_id, 'cup_google_short_url_id', $url_shortner->get_short_url_id());
	        }
